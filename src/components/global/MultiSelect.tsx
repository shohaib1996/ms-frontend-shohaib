...
type TProps = {
    ...
    searchable?: boolean;
    readOnly?: boolean;
    // New prop to distinguish between addition and removal
...
const MultiSelect = ({
    ...
    const handleRemove = (e: RMouseEvent<HTMLButtonElement>, v: string) => {
        e.stopPropagation();
        const newValue = value.filter((val) => v !== val);
    ...
    return (
        <div ref={menuRef} className='relative w-full'>
            <div
                ...
                className='border flex flex-wrap py-1 px-2 text-gray text-xs items-center gap-1 border-forground-border w-full bg-background min-h-10 rounded-md'
            >
                {value.length > 0 ? (
                    value.map((v, i) => (
                        <div
