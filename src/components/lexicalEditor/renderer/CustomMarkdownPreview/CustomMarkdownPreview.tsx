...
const preprocessText = (text?: string): string => {
    ...
    processed = processed.replace(/~~(.*?)~~/g, '<del>$1</del>');
    // Replace __ for underline with <u> tags
...
const CustomMarkdownPreview = ({
    ...
    // In the renderContent function of CustomMarkdownPreview
    const renderContent = () => {
        if (!text) {
            return null;
        }
        ...
        // If no mentions, just process the text normally
        ...
        // MODIFIED APPROACH: Split text into segments around mentions and process each segment
        const segments = [];
        let lastIndex = 0;

        mentions.forEach((mention, idx) => {
            // Process text before the mention
            if (mention.index > lastIndex) {
                const beforeText = text.substring(lastIndex, mention.index);
                if (beforeText) {
                    segments.push({
                        type: 'text',
                        content: preprocessText(beforeText).replace(
                            /<\/?p>/g,
                            '',
            ...
            lastIndex = mention.index + mention.match.length;
        });
        ...
        if (lastIndex < text.length) {
            ...
            if (afterText) {
                segments.push({
                    type: 'text',
                    content: preprocessText(afterText).replace(/<\/?p>/g, ''), // Remove paragraph tags
                });
            }
        }

        // Wrap everything in a single paragraph to maintain correct structure
        return (
            <p>
