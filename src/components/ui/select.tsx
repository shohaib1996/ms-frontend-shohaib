...
// Create a context to share search term and match tracking between components
...
>(
    (
        ...
        // Reset match count when search term changes
        React.useEffect(() => {
            setMatchCount(0);
        }, [searchTerm]);
        ...
        const resetMatches = React.useCallback(() => {
            setMatchCount(0);
        }, []);
        ...
        const showNoResults = searchTerm.trim() !== '' && matchCount === 0;

        return (
            <SelectSearchContext.Provider
                value={{ searchTerm, addMatch, resetMatches }}
            >
...
>(({ className, children, searchValue, ...props }, ref) => {
    const { searchTerm, addMatch } = React.useContext(SelectSearchContext);
    ...
    if (searchTerm && searchTerm.trim() !== '') {
        ...
        // If we get here, this item matches the search
        // Call addMatch after a small delay to ensure it happens after rendering
        React.useEffect(() => {
