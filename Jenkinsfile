                ...
                script{
                     ...
                     if(BRANCH == 'main'){
                         ...
                         withCredentials([string(credentialsId: 'doctl_token', variable: 'DO_API_KEY')]) {
                                ...
                                helm upgrade --install bh-frontend ./helm/bh-frontend -f ./helm/bh-frontend/values.staging.yaml --set image.tag=${BRANCH}-${VERSION}
                                '''
                            }
                              ...
                              withCredentials([string(credentialsId: 'doctl_token', variable: 'DO_API_KEY')]) {
                                ...
                                helm upgrade --install bh-frontend-prod ./helm/bh-frontend -f ./helm/bh-frontend/values.prod.yaml --set image.tag=${BRANCH}-${VERSION}
                                '''
                            }
